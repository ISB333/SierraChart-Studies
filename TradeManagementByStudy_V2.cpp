...
SCDLLName("Trade Management By Study V2")

SCSFExport scsf_TradeManagementByStudyv2(SCStudyInterfaceRef sc)
{
	// Setup References
	...
	if (sc.SetDefaults)	
	{
		sc.GraphName = "Trade Management by Study v2";
	...
	{
		sc.SetPersistentInt(3, 0), sc.SetPersistentInt(2, 0);
		return ;
	}
	else
		sc.SetPersistentInt(3, abs(PositionData.PositionQuantity));
	int actual_order_qty = sc.GetPersistentInt(3);
	int prev_order_qty = sc.GetPersistentInt(4);
	if (!prev_order_qty || prev_order_qty < actual_order_qty)
	{
		sc.SetPersistentInt(4, abs(PositionData.PositionQuantity));
		if (Input_DetailedLogging.GetYesNo())
			sc.AddMessageToLog("Order Quantity SET", 1);
		return ;
	}	
	else if (Input_DetailedLogging.GetYesNo())
	{
		SCString Buffer;
		Buffer.Format("Previous Order Quantity: %d       Actual Order Quantity: %d", prev_order_qty, actual_order_qty);
		sc.AddMessageToLog(Buffer, 0);
	}

	if (actual_order_qty < prev_order_qty)
	{
		if (Input_DetailedLogging.GetYesNo())
